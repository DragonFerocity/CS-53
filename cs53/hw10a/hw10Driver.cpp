...
int main()
{
  ...
  while(i < Iterations && (!Ralph.Graded() || !Possum.Graded()))
  {
    ...
    if(TheSchool.getNumTrash() != 0 && !Ralph.getWeaponStatus() 
        && !Possum.getWeaponStatus())
    {
      Ralph.move(TheSchool);
      Possum.move(TheSchool);
    }
    if (Ralph.getWeaponStatus())
    {
      Ralph.setLastMove(Possum.getLastMove());
      Ralph.moveWeapon(TheSchool);
      Possum.moveDefend(TheSchool);
    }
    if (Possum.getWeaponStatus())
    {
      Possum.setLastMove(Ralph.getLastMove());
      Possum.moveWeapon(TheSchool);
      Ralph.moveDefend(TheSchool);
    }
