...
{ 
  ...
  for (int i = 0; i < school.getYard_size(); i++)
  {
    ...
    if ((school.getPlot(i,m_x_coord) == 'T' 
        || school.getPlot(i,m_x_coord) == 'O')
...
///////////////////////////////////////////////////////////////////////////////
void Classmate::moveWeapon(Schoolyard& school)
{ 
  int Dir, mvX = 0, mvY = 0, Pos = 0;
  int LeastValue, ClosestTrash = 60, MvDir = 0;
  int numTrash = school.getNumTrash();
  ...
  for (int i = 0; i < school.getYard_size(); i++)
  {
    if (school.getPlot(m_y_coord,i) == 'C' && abs(i-m_x_coord) < ClosestTrash)
    {
      ClosestTrash = abs(i-m_x_coord);
      if (i < m_x_coord) //Left
        MvDir = -1;
    ...
    if (school.getPlot(i,m_x_coord) == 'C' && abs(i-m_y_coord) < ClosestTrash)
    {
      ClosestTrash = abs(i-m_y_coord);
      if (i < m_y_coord)//Up
        MvDir = -2;
  ...
  school.setPlot(m_y_coord,m_x_coord,'W');
  return;
}
///////////////////////////////////////////////////////////////////////////////
void Classmate::moveDefend(Schoolyard& school)
{ 
  int Dir, mvX = 0, mvY = 0, Pos = 0;
  int LeastValue, ClosestTrash = 60, MvDir = 0;
  int numTrash = school.getNumTrash();
  ...
  for (int i = 0; i < school.getYard_size(); i++)
  {
    if (school.getPlot(m_y_coord,i) == 'O' && abs(i-m_x_coord) < ClosestTrash)
    {
      ClosestTrash = abs(i-m_x_coord);
      if (i < m_x_coord) //Left
        MvDir = -1;
    ...
    if (school.getPlot(i,m_x_coord) == 'O' && abs(i-m_y_coord) < ClosestTrash)
    {
      ClosestTrash = abs(i-m_y_coord);
      if (i < m_y_coord)//Up
        MvDir = -2;
  ...
  if (ClosestTrash == 60)
  {
    do
    {
      ...
      if (Dir == 3)
        ...
        || m_y_coord+mvY < 0 || m_x_coord+mvX < 0
        || school.getPlot(m_y_coord+mvY, m_x_coord+mvX) == 'C'
        || school.getPlot(m_y_coord+mvY, m_x_coord+mvX) == 'W');
  ...
  if (school.getPlot(m_y_coord,m_x_coord) == 'O')
  {
    ...
    if (m_numItems == MAXTRASH)
    {
      ...
      if (Weapon.m_itemValue > LeastValue)
      {
        m_trash[Pos].m_trashValue = Weapon.m_itemValue;
        strcpy(m_trash[Pos].m_name,Weapon.m_name);
      }
    }
    else
    {
      m_trash[m_numItems].m_trashValue = Weapon.m_itemValue;
      strcpy(m_trash[m_numItems].m_name,Weapon.m_name);
      m_numItems++;
    }
    numTrash--;
    school.setNumTrash(numTrash);
